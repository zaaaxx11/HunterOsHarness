...
NOTICE = "A new version of hunter is available: 0.3.1 → 0.6.0. Run: hunter update"
RUNNER = CliRunner()
...
def test_release_version_sources_are_0_5_0():
...
def test_release_semver_and_update_tag_contract():
    ...
    assert "v0.3.1...v0.6.0" in changelog
...
def test_v05_roadmap_covers_maintenance_tracks():
    ...
    F10 untracks docs/ROADMAP-v0.5.md (owner directive: no roadmap on
    GitHub), so on a fresh clone the file is absent and there is nothing to
    assert — skip instead of failing.
    """
    roadmap_path = ROOT / "docs" / "ROADMAP-v0.5.md"
    if not roadmap_path.is_file():
        pytest.skip("docs/ROADMAP-v0.5.md is untracked by design (F10); absent on a fresh clone")
...
def test_current_docs_have_skills_and_curator_contract():
    ...
    docs = _docs_text()
    assert "~/.hunter/skills/<name>/SKILL.md" in docs
...
def test_current_docs_have_no_stale_release_claims():
    """A11: current examples show the CURRENT release and not obsolete M3/M5 copy.

    Version-claim presence is asserted where release claims live (README,
    QUICKSTART); docs/FIRST-RUN.md carries illustrative UI banners that are
    not release claims, so it is covered by the stale-copy checks only.
    """
    ...
    assert "25 slash commands" in current.lower()
...
def test_ci_matrix_and_commands_are_explicit():
    """A12: CI declares the matrix jobs, selected-interpreter commands, and
    the F10 tests-optional guard (fresh clones ship without tests/)."""
    ...
    # Portability: the supported floor (3.10) and the newest supported CPython
    # are both exercised, so the declared requires-python is actually tested.
    ...
    commands = [step.get("run", "") for step in run_steps]
    assert 'python -m pip install -e ". [dev]"' not in commands
    assert 'python -m pip install -e ". [browser]"' not in commands
    assert any('python -m pip install -e ".[dev]"' in command for command in commands)
    # One bash syntax for the guarded steps: ruff and pytest pin shell: bash
    # (the windows-latest default would be pwsh).
    for name in ("Ruff", "Pytest"):
        step = next(step for step in run_steps if step.get("name") == name)
        assert step.get("shell") == "bash", name
    ruff = next(command for command in commands if "python -m ruff check" in command)
    # Ruff always covers src + scripts; tests only when the directory exists.
    assert "python -m ruff check src scripts" in ruff
    assert "[ -d tests ]" in ruff and "tests" in ruff.split("ruff check", 1)[1]
    pytest_step = next(command for command in commands if "python -m pytest" in command)
    assert "[ -d tests ]" in pytest_step  # guarded: skips cleanly without tests/
    assert "ruff check src tests" not in "".join(commands)
...
def test_ci_has_no_browser_install_or_network_test_path():
    ...
    assert "python -m ruff check src scripts" in workflow
...
def test_ci_four_job_quality_gate_contract():
    ...
    plan = _read("docs/plans/v0.4/M7-release-maintenance.md")
    # F10 shape: ruff covers src + scripts always, tests when present; pytest
    # is guarded on the untracked-by-design tests/ directory.
    assert "python -m ruff check src scripts" in workflow
    assert "[ -d tests ]" in workflow
...
def test_update_release_path_is_github_first_and_hermetic(tmp_path):
    ...
    def github_ok(request: httpx.Request) -> httpx.Response:
        ...
        if str(request.url) == update_core.GITHUB_RELEASES_URL:
            ...
            return httpx.Response(200, json={"tag_name": "v0.6.0"})
