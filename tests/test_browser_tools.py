...
def test_registry_adds_exact_browser_tools_only_when_available():
    ...
    # M8 drift: runtime_inventory joins the basic tier (15 → 16).
    assert len(default_registry.schemas_for_tier("basic")) == 16
...
def test_tool_context_owns_one_lazy_session_and_idempotent_cleanup(tmp_path):
    ...
    try:
        ...
        # M8 F5 cloak drift: the default session launches WITH cloak kwargs —
        # headless still True and the pinned AutomationControlled arg present
        # (not the old exact single-kwarg equality).
        assert len(driver.launch_calls) == 1
        launch_kwargs = driver.launch_calls[0]
        assert launch_kwargs.get("headless") is True
        assert "--disable-blink-features=AutomationControlled" in (launch_kwargs.get("args") or [])
        context_kwargs = driver.browser.new_context_calls[0]
        assert set(context_kwargs) == {"user_agent", "viewport", "locale", "timezone_id"}
