...
def test_findings_only_status_and_evidence_ids_mutable(tmp_path, make_ledger):
    ...
    ev_r2 = led.add_evidence("r2", "http_response", {"body": "other-run"})
    ...
    try:
        ...
        # RULE-E1 at storage: empty, dangling, and cross-run rewrites fail.
        with pytest.raises(sqlite3.IntegrityError):
        ...
        # Only status and valid same-run evidence_ids may change.
...
def test_default_state_dir_and_env_override(tmp_path, monkeypatch):
    """M11 M1 (§14 item 3): the default state dir is ~/.hunter (HOME-isolated),
    never cwd; $HUNTER_STATE_DIR still wins."""
    monkeypatch.setenv("HOME", str(tmp_path / "home"))
    monkeypatch.setenv("USERPROFILE", str(tmp_path / "home"))
    monkeypatch.chdir(tmp_path)  # a cwd ./.hunter must NEVER be the default
    led = Ledger()
    try:
        led.create_run("r1", "t", "e", "s")
        assert (tmp_path / "home" / ".hunter" / "ledger.db").exists()
