...
def test_ask_seam_fail_closed_never_saves_on_error(tmp_path):
    ...
    for ask in asks:
        ...
        assert not (home / ".hunter" / "skills").exists()
...
def test_preview_contains_verdict_flags_and_body(tmp_path):
    ...
    assert str(tmp_path / ".hunter" / "skills" / "preview-card" / "SKILL.md") in text
...
def test_curate_surfaces_end_to_end(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert list((tmp_path / ".hunter" / "skills").glob("*/SKILL.md"))
    finally:
        store.close()
        ledger.close()
    ...
    assert not (cli_home / ".hunter" / "skills").exists()
