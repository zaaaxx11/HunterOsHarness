...
def test_provider_error_keeps_repl_alive(store, monkeypatch):
    ...
    # M11 M3 (§14 item 10): the per-line error text is replaced by the ONE
    # setup panel (first turn) and the short line (later turns).
    ...
    console = FakeConsole(["hello", "and again", "/quit"])
    run_repl(engine=engine, console=console)
    text = console.text()
    assert "ERROR config" in text and "no config in tests" in text
    assert text.count("Fix it in under a minute") == 1  # the panel, exactly once
    assert "💡 no brain configured — run hunter init" in text  # short line afterwards
...
def test_audit_conversational_turns_are_governed(store, tmp_path):
    ...
    it gets a normal reply + hint (repl.py:59-64, :366-509). Arming here goes
    through explicit hunt_mode on this surface, so the governed-turn asserts
    keep testing the audit path without weakening the firewall."""
