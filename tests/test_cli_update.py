...
def test_update_dev_method_advice_only(monkeypatch, tmp_path):
    ...
    # The advice must be runnable in the CURRENT shell: PowerShell 5.1 rejects
    # `&&`, so Windows gets two lines while POSIX keeps the one-liner.
    if os.name == "nt":
        assert "git pull" in result.output
        assert "-m pip install -e ." in result.output
