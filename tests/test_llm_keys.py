...
def test_write_and_parse_keys_env_roundtrip(tmp_path):
    ...
    assert written == tmp_path / ".hunteros" / KEYS_ENV_FILENAME
...
def test_keys_env_path_env_override_and_default(tmp_path):
    """A22: $HUNTEROS_KEYS_FILE overrides; default is <home>/.hunteros/keys.env."""
    ...
    assert keys_env_path(env={}, home=tmp_path) == tmp_path / ".hunteros" / KEYS_ENV_FILENAME
...
def test_load_keys_env_missing_file_is_noop(tmp_path):
    ...
    assert not (tmp_path / ".hunteros").exists()
...
def test_cli_startup_loads_keys_env_and_resolve_key_sees_it(monkeypatch, tmp_path):
    ...
    keys_path = tmp_path / ".hunteros" / KEYS_FILENAME
